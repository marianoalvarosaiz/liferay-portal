	...
	<li class="control-menu-nav-item">
		...
		<div class="dropdown">
			<a class="dropdown-toggle layout-set-branch-selector staging-variation-selector" data-toggle="dropdown" href="#1">
				<liferay-ui:message key="<%= HtmlUtil.escape(layoutSetBranch.getName()) %>" localizeKey="<%= translateLayoutSetBranchName(layoutSetBranch.getName()) %>"/>
				...
				%>
					...
					<li>
						<a class="<%= selected ? "disabled" : StringPool.BLANK %>" href="<%= selected ? "javascript:;" : "javascript:submitForm(document.hrefFm, '" + HtmlUtil.escapeJS(curLayoutSetBranchURL) + "');" %>">
							<liferay-ui:message key="<%= HtmlUtil.escape(curLayoutSetBranch.getName()) %>" localizeKey="<%= translateLayoutSetBranchName(curLayoutSetBranch.getName())%>" />
						</a>
					</li>
