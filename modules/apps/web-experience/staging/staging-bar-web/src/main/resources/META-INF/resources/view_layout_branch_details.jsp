...
<li class="control-menu-nav-item">
	...
	<div class="dropdown">
		<a class="dropdown-toggle layout-branch-selector staging-variation-selector" data-toggle="dropdown" href="#1">
			<liferay-ui:message key="<%= HtmlUtil.escape(layoutBranch.getName()) %>" localizeKey="<%= translateLayoutBranchName(layoutBranch.getName()) %>" />
			...
			%>
				...
				<li>
					<a class="<%= selected ? "disabled" : StringPool.BLANK %>" href="<%= selected ? "javascript:;" : curLayoutBranchURL %>">
						<liferay-ui:message key="<%= HtmlUtil.escape(curLayoutBranch.getName()) %>" localizeKey="<%= translateLayoutBranchName(curLayoutBranch.getName()) %>" />
					</a>
				</li>
