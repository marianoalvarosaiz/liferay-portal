...
public class PostupgradeVerifyDatabaseStateTest
	...
	@Test
	public void testVerifyPostupgradeCustomerTable() throws Exception {
		...
		try (LogCapture logCapture = LoggerTestUtil.configureLog4JLogger(
			...
			Assert.assertTrue(message, message.contains("non-Liferay table"));
		}
		finally {
	...
	public void testVerifyPostupgradeMissingNonserviceBuilderTable()
		...
		try (LogCapture logCapture = LoggerTestUtil.configureLog4JLogger(
			...
			Assert.assertEquals(
				_getFinding("Missing tables were detected", tableName),
	...
	public void testVerifyPostupgradeRequireSchemaVersionBundleTable()
		...
		Assert.assertEquals(
			...
			tableOwnerMap.get("Configuration_"));
	...
	public void testVerifyPostupgradeStaleTable() throws Exception {
		...
		try {
			try (LogCapture logCapture = LoggerTestUtil.configureLog4JLogger(
				...
				Assert.assertEquals(
					_getFinding("Stale tables were detected", tableName),
					_getMessage(logCapture, tableName));
			}
			...
			try (LogCapture logCapture = LoggerTestUtil.configureLog4JLogger(
				...
				String message = _getMessage(logCapture, "incomplete upgrade");
				...
				Assert.assertTrue(
					...
					message.startsWith(
						StringBundler.concat(
							...
							" (incomplete upgrade): release state upgrade ",
							"failure, schema version 1.0.0, last modified ")));
			}
		}
		finally {
			if (release != null) {
	...
	public void testVerifyPostupgradeViews() throws Exception {
		...
		String backupViewName = getNormalizedName(_BACKUP_VIEW_NAME);

		renameView(_VIEW_NAME, _BACKUP_VIEW_NAME);
		...
		try {
			try (LogCapture logCapture = LoggerTestUtil.configureLog4JLogger(
				...
				String message = _getMessage(
					logCapture, "Customer or non-Liferay views");

				Assert.assertTrue(message, message.contains(backupViewName));
			}

			serviceComponent = _addServiceComponent(_BACKUP_VIEW_NAME);
			...
			try (LogCapture logCapture = LoggerTestUtil.configureLog4JLogger(
				...
				Assert.assertEquals(
					_getFinding("Stale views were detected", backupViewName),
			...
			renameView(_BACKUP_VIEW_NAME, _VIEW_NAME);
		}
	}
	...
	private String _getFinding(String prefix, String tableName)
		throws Exception {
	...
	private static final String _BACKUP_VIEW_NAME = "VirtualHost_backup";
