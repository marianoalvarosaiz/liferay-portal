...
public class DBImporter {
	...
	public DBImporter(
		String path, DataSource targetDataSource, DBType targetDBType) {

		_path = path;
		_targetDataSource = targetDataSource;

		_sourceDataSource = InfrastructureUtil.getDataSource();

		_targetDB = DBManagerUtil.getDB(targetDBType, targetDataSource);

		_dbConcurrentExecutor = new DBConcurrentExecutor(
			_targetDB,
			sql ->
				StringUtil.count(
	...
	public void importDB() throws Exception {
		_dbConcurrentExecutor.runSQLTemplateConcurrently(
			_targetDataSource, FileUtil.read(new File(_path, "tables.sql")));
		...
		_dbConcurrentExecutor.runSQLTemplateConcurrently(
			_targetDataSource, FileUtil.read(new File(_path, "indexes.sql")));

		_dbConcurrentExecutor.stop();
	}
	...
	private void _loadColumnsMetadata(
		...
		try (Connection connection = dataSource.getConnection()) {
			...
			try (ResultSet resultSet = databaseMetaData.getColumns(
					connection.getCatalog(), connection.getCatalog(), null,
					null)) {
				...
				while (resultSet.next()) {
					...
					columnsName.add(resultSet.getString(columnName));
	...
	private final DBConcurrentExecutor _dbConcurrentExecutor;
